...
export async function POST(request: Request) {
  try {
    ...
    if (!Array.isArray(students)) {
      return NextResponse.json({ detail: "Expected an array of students" }, { status: 400 });
    }

    const backendUrl = process.env.BACKEND_URL || "http://127.0.0.1:8000";

    // Detect if payload is already in 30-feature model format
    const isModelFormat = students.length > 0 && ("Age" in students[0] || "Gender" in students[0]);
    const fastApiPayload = isModelFormat ? students : students.map(transformPayload);

    const response = await fetch(`${backendUrl}/predict/bulk`, {
      method: "POST",
      headers: {
    ...
    const data = await response.json(); // Array of predictions
